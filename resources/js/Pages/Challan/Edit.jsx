...
import { MdAdd, MdSave, MdArrowBack, MdRemove, MdInventory, MdWarning, MdInfo, MdEdit, MdPerson } from 'react-icons/md';
...
export default function ChallanEdit(props) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        total_lots: challan.total_lots || 0
    });

    // Initialize groups from existing challan items
    useEffect(() => {
        if (challan.items && challan.items.length > 0) {
            ...
            const initialGroups = Object.keys(groupedItems).map((groupNumber) => {
                const items = groupedItems[groupNumber].sort((a, b) => a.sr_number - b.sr_number);
                const groupItems = Array.from({ length: 12 }, (_, i) => {
                    const existingItem = items.find(item => item.sr_number === (items[0].sr_number + i));
                    return {
                        sr: items[0].sr_number + i,
                        meter: existingItem ? existingItem.meter : ''
                    };
                });

                return {
                    id: parseInt(groupNumber),
                    items: groupItems
                };
            });
            ...
            setGroups([{ id: 1, items: Array.from({ length: 12 }, (_, i) => ({ sr: i + 1, meter: '' })) }]);
        }
    }, [challan]);
    ...
    const calculateTotals = () => {
        ...
        // Check stock availability (including current challan's meters)
        const currentChallanMeters = parseFloat(challan.total_meter) || 0;
        const effectiveAvailableStock = availableStock + currentChallanMeters;

        if (totalMeter > effectiveAvailableStock) {
            setStockAlert(`Stock is not available. Available: ${effectiveAvailableStock}m, Required: ${totalMeter}m`);
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('challans.update', challan.id));
    };

    const breadcrumbs = [
        { label: 'Dashboard', href: route('dashboard') },
        { label: 'Challan Management', href: route('challans.index') },
        { label: `Edit Challan ${challan.formatted_challan_number}` }
    ];

    return (
        <>
            <Head title={`Edit Challan ${challan.formatted_challan_number}`} />
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 lg:mb-8 space-y-3 sm:space-y-0">
                            <div className="flex items-center space-x-3">
                                {/* <div className="p-2 bg-secondary/10 rounded-lg">
                                    <MdEdit className="w-6 h-6 text-secondary" />
                                </div> */}
                                <div>
                                    <h1 className="text-lg sm:text-xl lg:text-2xl font-bold text-text">
                                        Edit Challan {challan.formatted_challan_number}
                                    </h1>
                                    <p className="text-sm text-gray-600 mt-1">Update challan details and entries</p>
                                </div>
                            </div>
                            <Button
                                variant="secondary"
                                    ...
                                    <div className="flex items-center space-x-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xl font-bold text-primary">
                                                {availableStock + parseFloat(challan.total_meter || 0)} meters
                                            </p>
                                            <p className="text-xs text-gray-500">Including current challan</p>
                                        </div>
                                    </div>
                                </Card>
                            ...
                            <div className="space-y-4 sm:space-y-6">
                                        ...
                                        <div className="p-4 bg-white rounded-lg shadow-sm border border-primary/20">
                                            <div className="flex items-center space-x-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="text-xl sm:text-2xl font-bold text-primary">{(parseFloat(data.total_meter) || 0).toFixed(2)} m</div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div className="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3">
                                    ...
                                    >
                                        ...
                                        <span>{processing ? 'Updating...' : 'Update Challan'}</span>
                                    </Button>
                                </div>
                            </div>
                        </form>
